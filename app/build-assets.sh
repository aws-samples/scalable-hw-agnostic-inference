...
if [ "$IMAGE_TAG" == "amd64-neuron" ]; then
  docker logout
  aws ecr get-login-password --region us-west-2 | docker login --username AWS --password-stdin $DLC_NEURON_ECR/$DLC_NEURON_IMAGE:$DLC_NEURON_TAG
...
if [ "$IMAGE_TAG" == "amd64-cuda" ]; then
  docker logout
  aws ecr get-login-password --region us-east-1 | docker login --username AWS --password-stdin $DLC_CUDA_ECR/$DLC_CUDA_IMAGE:$DLC_CUDA_TAG
